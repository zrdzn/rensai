...
public class EventCoverageScannerTest {
    ...
    public void generateMissingEventsReport() {
        ...
        Set<Class<? extends Event>> allEvents = paperReflections.getSubTypesOf(Event.class).stream()
                .filter(clazz -> !Modifier.isAbstract(clazz.getModifiers()))
                .collect(Collectors.toSet());

        Reflections agentReflections = new Reflections("dev.rensai.agent.paper.listener", Scanners.MethodsAnnotated);
        ...
        List<String> missingEvents = allEvents.stream()
                .map(Class::getSimpleName)
                .filter(name -> !implementedEvents.contains(name))
                .sorted()
                .toList();

        System.out.println("Implemented events (" + implementedEvents.size() + "/" + allEvents.size() + "):");
